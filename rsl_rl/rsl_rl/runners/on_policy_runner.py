...
class OnPolicyRunner:
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        # === 新增：把 termination 统计成“占比” ===
        if locs["ep_infos"]:
            # 找出所有 Episode_Termination/* 的 key
            term_keys = [
                k for k in locs["ep_infos"][0].keys()
                if isinstance(k, str) and k.startswith("Episode_Termination/")
            ]
            if term_keys:
                term_totals = {k: 0.0 for k in term_keys}
                total_terminations = 0.0

                # 累加这次 logging 窗口内，各种 termination 的总次数
                for ep_info in locs["ep_infos"]:
                    for k in term_keys:
                        if k not in ep_info:
                            continue
                ...
                # 计算各 termination 占总终止数的比例，并记录到 wandb
                if total_terminations > 0:
                    for k, tot in term_totals.items():
                        rate = tot / total_terminations
                        rate_key = k.replace(
                            "Episode_Termination/", "Episode_Termination_Rate/"
                        )
                        self.writer.add_scalar(rate_key, rate, locs["it"])
                        ep_string += f"""{f'{rate_key}:':>{pad}} {rate:.4f}\n"""
        ...
        header_text = f" \033[1m Learning iteration {locs['it']}/{locs['tot_iter']} \033[0m "

        if len(locs["rewbuffer"]) > 0:
            log_string = (
                f"""{'#' * width}\n"""
                f"""{header_text.center(width, ' ')}\n\n"""
