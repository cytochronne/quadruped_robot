...
protected:
    void _prapare_terms()
    {
        ...
        for(auto it = this->cfg.begin(); it != this->cfg.end(); ++it)
        {
            ...
            if(key == "scale_first" || key == "use_gym_history" || key == "enable_noise") {
                continue;
            }
            only_one_input = it->second["params"].IsDefined(); // a term has params field
            break;
        }
        ...
        if(only_one_input) {
            ...
            for(auto group = this->cfg.begin(); group != this->cfg.end(); ++group)
            {
                ...
                if(group_name == "scale_first" || group_name == "use_gym_history" || group_name == "enable_noise") {
                    continue;
                }
    ...
    std::vector<ObservationTermCfg> _prepare_group_terms(const YAML::Node & group_cfg)
    {
        ...
        bool enable_noise = false; // default off
        for(auto it = group_cfg.begin(); it != group_cfg.end(); ++it)
        {
            ...
            if(it->first.as<std::string>() == "enable_noise") {
                enable_noise = it->second.as<bool>();
                continue;
            }
            ...
            term_cfg.enable_noise = enable_noise;
            ...
            if(term_yaml_cfg["noise_type"].IsDefined() && !term_yaml_cfg["noise_type"].IsNull()) {
                term_cfg.noise_type = term_yaml_cfg["noise_type"].as<std::string>();
            }
            if(term_yaml_cfg["noise_n_min"].IsDefined() && !term_yaml_cfg["noise_n_min"].IsNull()) {
                term_cfg.noise_n_min = term_yaml_cfg["noise_n_min"].as<float>();
            }
            if(term_yaml_cfg["noise_n_max"].IsDefined() && !term_yaml_cfg["noise_n_max"].IsNull()) {
                term_cfg.noise_n_max = term_yaml_cfg["noise_n_max"].as<float>();
            }
            if(!term_cfg.noise_type.empty() && term_cfg.noise_type != "uniform") {
                throw std::runtime_error("Unsupported observation noise_type '" + term_cfg.noise_type + "'.");
            }
