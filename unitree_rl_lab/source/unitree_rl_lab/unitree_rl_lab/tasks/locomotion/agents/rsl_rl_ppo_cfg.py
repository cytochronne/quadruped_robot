...
@configclass
class RecurrentPpoActorCriticCfg(RslRlPpoActorCriticCfg):
...
@configclass
class TerrainAwarePPOAlgorithmCfg(RslRlPpoAlgorithmCfg):
...
@configclass
class StudentPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 50000
    save_interval = 100
    experiment_name = ""  # same as task name
